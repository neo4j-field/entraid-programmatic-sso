...
def get_sso_token():
    ...
    if "access_token" in result:
        return result['expires_in'], result["access_token"]
    else:
        raise Exception(f"Failed to get token: {result.get('error_description')}")

def auth_provider():

    # Get our token and its expiration time in seconds
    expires_in, sso_token = get_sso_token()

    # Include a little buffer so that new token is fetched before the old one expires
    expires_in -= 10
    
    auth = bearer_auth(sso_token)
